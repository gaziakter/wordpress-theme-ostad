...
$format_url = function_exists('get_field') ? get_field('post_formate') : null;

?>

<?php if(is_single()) :  ?>
    <article id="post-<?php the_id(); ?>" <?php post_class('tp-format-standard dsd'); ?>>
...
<article id="post-<?php the_id(); ?>" <?php post_class('tp-format-video postbox__item format-image mb-50 transition-3'); ?>>
    <?php if(has_post_thumbnail()) : ?>
        <div class="postbox__thumb postbox__video w-img p-relative mb-20">
            <a href="<?php the_permalink( ); ?>">
                <?php the_post_thumbnail(); ?>
            </a>
            <?php if(!empty($format_url)) : ?>
